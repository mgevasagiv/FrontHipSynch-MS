...
formula = 'mg ~ event * ripple_occur + blk * ripple_occur + (ripple_occur|epair)';
...
formula = 'impv ~ mg * event  + (epair|subj)';
lme1 = fitlme(data,formula);
formula = 'impv ~ mg * event * blk  + (epair|subj)';
lme2 = fitlme(data,formula);
results = compare(lme1,lme2);
disp(results.pValue)


data_i = data(data.impv == 1,:);
data_i = data_i( strcmp(data_i.event,'preG'),:) ;
interactionplot(data_i.mg,[data_i.blk, data_i.OFC_axis, data_i.ripple_occur],'varnames',{'B','O','R'})

data_i = data(data.impv == 1,:);
data_i = data_i( strcmp(data_i.event,'G'),:) ;
interactionplot(data_i.mg,[data_i.blk, data_i.OFC_axis, data_i.ripple_occur],'varnames',{'B','O','R'})

data_i = data(data.impv == 1,:);
data_i = data_i( strcmp(data_i.event,'X'),:) ;
interactionplot(data_i.mg,[data_i.blk, data_i.OFC_axis, data_i.ripple_occur],'varnames',{'B','O','R'})
    
%%
fig_name = 'cohens_effectSize_impv';
newA4figure(fig_name)
...
for ii = 1:length(EvTypePlot)
    ...
    data_i = data(data.impv == 1,:);
    data_i = data_i( strcmp(data_i.event,EvTypePlot{ii}),:) ;
    formula = 'mg ~ ripple_occur * blk + (ripple_occur|epair)';
    lme1 = fitlme(data_i,formula);

    ano = anova(lme1);
    pval(1,ii) =  ano.pValue(2);
    pval(2,ii) =  ano.pValue(3);
    pval(3,ii) =  ano.pValue(4);
  
    FStat(1,ii) =  ano.FStat(2);
    FStat(2,ii) =  ano.FStat(3);
    FStat(3,ii) =  ano.FStat(4);
    
    data = data(~isnan(data.mg),:);
    data_x = data.mg( data.impv == 1 & data.blk == 1 & data.ripple_occur == 1 & ... 
                            strcmp(data.event,EvTypePlot{ii}) );

    data_y = data.mg( data.impv == 0 & data.blk == 1 & data.ripple_occur == 1 & ... 
                            strcmp(data.event,EvTypePlot{ii}) );
    ...
    ax = get(gca);
    set(gca,'xticklabel',{'Improved','Degraded', 'Robust Cohen''s d'},'ylim',[0 20], 'YTick',[0 20])
